<?php include $_SERVER['DOCUMENT_ROOT'] . "/Mega-Triunfo/Projeto/public/html/partials/head.php"; ?>

<body class='d-flex flex-column vh-100 align-content-center justify-content-around text-center overflow-start'>

  <?php include $_SERVER['DOCUMENT_ROOT'] . "/Mega-Triunfo/Projeto/public/html/partials/header.php"; ?>

  <main class='container d-flex justify-content-center align-items-center'>
    <!-- <div class='bg-white container-shadow container-size w-100 p-4'> -->
    <div class='row bg-white container-shadow container-size w-100'>
      <div class='col-6 container-card py-2'>
        <form class='container d-flex flex-wrap justify-content-around h-98' id="deck-table">
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>
          <div class="d-flex flex-column-reverse">
            <img class="pt-1 pb-2 rounded border-card" id="" src="https://via.placeholder.com/110x150px">
            <label for="img1">Nome Baralho</label>
          </div>

        </form>
      </div>
      <div class='col-3 container-card'>
        <div class='container d-flex flex-wrap flex-column justify-content-center h-100 align-items-center'>
          <img class='img-fluid mt-4 d-block rounded' src='https://via.placeholder.com/300x400'>
          <span class='h4 mt-3'>Nome do Baralho</span>
        </div>
      </div>
      <div class='col-3 container-card'>
        <div class='container d-flex flex-wrap flex-column justify-content-center align-items-center'>
          <button id='btn-new-card' class='manage-card-buttons'>
            Novo Baralho
            <span class='manage-card-buttons-border'></span>
          </button>
          <button id='btn-new-car' class='manage-card-buttons'>
            Editar Baralho
            <span class='manage-card-buttons-border'></span>
          </button>
        </div>

      </div>
    </div>
</main>
</body>

</html>